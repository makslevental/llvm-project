...
ADVERT_BEGIN = "{comment_delimiter} NOTE: Assertions have been autogenerated by "
ADVERT_END = """
{comment_delimiter} The script is designed to make adding checks to
{comment_delimiter} a test case fast, it is *not* designed to be authoritative
{comment_delimiter} about what constitutes a good test! The CHECK should be
{comment_delimiter} minimized and named to reflect the test intent.
"""
...
def process_attribute_definition(args, line, attribute_namer, output):
    m = ATTR_DEF_RE.match(line)
    if m:
        attribute_name = attribute_namer.generate_name(m.group(1))
        line = args.comment_delimiter + ' ' + args.check_prefix + ': #[[' + attribute_name + ':.+]] =' + line[len(m.group(0)):] + '\n'
        output.write(line)
...
def main():
    ...
    parser.add_argument(
        "--comment-delimiter", default="//", help="String that delimits comment."
    )
    parser.add_argument(
    ...
    autogenerated_note = ADVERT_BEGIN.format(comment_delimiter=args.comment_delimiter) + "utils/" + script_name + "\n" + ADVERT_END.format(comment_delimiter=args.comment_delimiter)
    ...
    for input_line in input_lines:
        ...
        process_attribute_definition(args, input_line, attribute_namer, output)
        ...
        if is_block:
            input_line = input_line.rsplit(args.comment_delimiter, 1)[0].rstrip()
        ...
        if len(output_segments[-1]) != 0 or not ssa_split[0]:
            output_line = args.comment_delimiter + " " + args.check_prefix + ": "
            ...
            # label.
            output_line = args.comment_delimiter + " " + args.check_prefix + "-LABEL: " + ssa_split[0] + "\n"

            # Process the rest of the input line on separate check lines.
            for argument in ssa_split[1:]:
                output_line += args.comment_delimiter + " " + args.check_prefix + "-SAME:  "
